...
export default function TopProductsChart() {
  ...
  const formattedTopProducts = useMemo(() => {
    ...
    return topProducts.map((prod, idx) => ({
      ...prod,
      percentage: Math.round((prod.total_quantity / maxQty) * 100),
      ...(colors[idx] || colors[4])
    }));
  }, [topProducts]);
              ...
              <div key={product.id} className="space-y-2">
                <div className="flex items-center justify-between text-[12px]">
                  ...
                  <div className="text-right whitespace-nowrap pl-3">
                    <span className="font-bold text-[#0b1c30]">{product.total_quantity} x</span>
                    <span className="text-[#76777d] mx-1.5">•</span>
                    <span className="text-[#76777d]">Rp {formatCurrency(product.total_revenue)}</span>
                  </div>
                </div>
